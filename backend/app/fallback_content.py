...
The learner's flow never breaks: concept maps, lessons, quizzes, and tutor
replies all have offline equivalents. Fallback content is clearly generic,
but it keeps every workflow demonstrable end-to-end.
"""
...
_CONCEPT_TEMPLATE = [
    ("foundations", "Foundations of {topic}", "Key terminology, motivation, and the mental model behind {topic}.", "easy", []),
    ("core-principles", "Core Principles", "The central rules and ideas that everything else in {topic} builds on.", "easy", ["foundations"]),
    ("techniques", "Essential Techniques", "The standard methods and procedures used when working with {topic}.", "medium", ["core-principles"]),
    ("applications", "Practical Applications", "Applying {topic} to realistic problems and interpreting results.", "medium", ["techniques"]),
    ("pitfalls", "Common Pitfalls & Misconceptions", "Frequent mistakes, edge cases, and how experts avoid them in {topic}.", "medium", ["techniques"]),
    ("advanced", "Advanced Topics", "Deeper extensions and open questions that build on all of {topic}.", "hard", ["applications", "pitfalls"]),
]
...
def lesson(concept_name: str, description: str, band: str, topic: str) -> str:
    ...
    }.get(band, "Work through this step by step.")
    return f"""## Why this matters

> Offline study guide (Gemini unavailable — add `GEMINI_API_KEY` to `.env` for fully personalized lessons).

**{concept_name}** — {description} Understanding it unlocks the concepts that build on it in *{topic}*.
...
```flow
{{"title": "The Medhā mastery loop for {concept_name}", "steps": ["Define it in your own words", "Connect it to two other ideas in {topic}", "Apply it to one concrete example", "Take the adaptive quiz", "Review what you missed"], "note": "Follow this loop top to bottom; each pass through it raises your mastery estimate."}}
```

```chart
{{"type": "line", "title": "How practice builds mastery", "x_label": "Practice sessions", "y_label": "Mastery estimate (%)", "labels": ["1", "2", "3", "4", "5"], "series": [{{"name": "Steady practice", "values": [15, 40, 62, 78, 88]}}, {{"name": "Cramming once", "values": [15, 55, 50, 45, 41]}}], "note": "Read left to right: repeated spaced practice keeps climbing, while a single cram session fades. This is the forgetting curve Medhā's review scheduler works against."}}
```

## Practical example

Take one real situation from your own life or work and force *{concept_name}* onto it:
if you can explain how the concept applies (or why it doesn't), you understand it.
...
def quiz(concept_name: str, description: str, difficulty: str, count: int) -> list[dict[str, Any]]:
    ...
    bank = [
        ...
        {
            "question": f"You keep making mistakes while practicing '{concept_name}'. What is the most effective next step?",
            "options": [
            ...
            "correct_index": 0,
            "explanation": "Deliberate practice means stepping down difficulty to rebuild the shaky foundation, then climbing back up.",
        },
        {
            "question": f"What is the strongest evidence that you have truly mastered '{concept_name}'?",
            "options": [
            ...
            "correct_index": 0,
            "explanation": "Transfer to novel problems plus articulate reasoning is the gold standard of mastery — recognition and recall are weaker signals.",
        },
        {
            "question": f"When is the best time to review '{concept_name}' after first learning it?",
            "options": [
            ...
            "correct_index": 0,
            "explanation": "Spaced repetition — reviewing near the edge of forgetting — is what Medhā's decay model schedules for you.",
        },
        {
            "question": f"A peer asks you to explain '{concept_name}'. Which explanation style helps BOTH of you learn most?",
            "options": [
            ...
            "correct_index": 0,
            "explanation": "Explaining with analogy and example (the Feynman technique) exposes gaps in your own understanding while teaching them.",
        },
    ]
    picked = []
...
def mindmap(concept_name: str, description: str, topic: str) -> dict:
    """A generic but well-structured mind map when Gemini is unavailable."""
    return {
        "center": concept_name[:50],
        "branches": [
            {"label": "What it is", "kind": "intuition",
             "children": [description[:70], f"A building block of {topic}"[:70]]},
            {"label": "Practical example", "kind": "example",
             "children": ["Apply it to one real problem you know",
                          "Explain it with a daily-life analogy"]},
            {"label": "How to use it", "kind": "steps",
             "children": ["Define it in your own words",
                          "Work one small example by hand",
                          "Check the result makes sense"]},
            {"label": "Common pitfalls", "kind": "pitfall",
             "children": ["Memorizing without understanding",
                          "Skipping the prerequisites"]},
            {"label": "Connects to", "kind": "connection",
             "children": [f"Other concepts in {topic}"[:70], "Your next unlocked concept"]},
        ],
    }


def teachback_grade(explanation: str, description: str) -> dict:
