/* Medhā — radial mind-map renderer.
...
(function () {
  ...
  async function open(concept) {
    const { api, state, loader, toast, show } = window.Medha;
    ...
    try {
      ...
      document.getElementById("mindmap-title").textContent = `Mind map · ${data.concept.name}`;
      const badge = document.getElementById("mindmap-source");
      badge.textContent = data.source === "cache" ? "cached" : data.source;
      badge.className = "badge " + (data.source === "fallback" ? "warn" : "ok");
      render(document.getElementById("mindmap-container"), data.mindmap);
      renderLegend(document.getElementById("mindmap-legend"), data.mindmap);
