/* Medhā — deterministic micro-visualization engine.
 *
...
window.MedhaViz = (function () {
  ...
  function validateChart(spec) {
    ...
    const series = (Array.isArray(spec.series) ? spec.series : [])
      ...
      .filter((entry) => entry.values.length === labels.length && entry.values.every((v) => v !== null));
  ...
  function chartSVG(chart) {
    ...
    if (chart.title) {
      parts.push(`<text x="${width / 2}" y="22" text-anchor="middle" class="viz-title">${esc(chart.title)}</text>`);
    }
    ...
    if (chart.yLabel) {
      parts.push(
        `<text x="16" y="${margin.top + plotHeight / 2}" class="viz-axis" text-anchor="middle" ` +
        `transform="rotate(-90 16 ${margin.top + plotHeight / 2})">${esc(chart.yLabel)}</text>`
      );
    }
    if (chart.xLabel) {
      parts.push(
        `<text x="${margin.left + plotWidth / 2}" y="${height - 8}" text-anchor="middle" class="viz-axis">` +
        `${esc(chart.xLabel)}</text>`
      );
    }
    ...
    chart.labels.forEach((label, index) => {
      ...
      parts.push(
        `<text x="${x}" y="${margin.top + plotHeight + 18}" text-anchor="middle" class="viz-tick">` +
        `${esc(label)}</text>`
      );
    });
    ...
    if (chart.type === "bar") {
      ...
      chart.series.forEach((entry, seriesIndex) => {
        ...
        const points = entry.values
          ...
          .join(" ");
        parts.push(`<polyline points="${points}" fill="none" stroke="${color}" stroke-width="3" stroke-linejoin="round"/>`);
        entry.values.forEach((value, index) => {
          parts.push(
            `<circle cx="${margin.left + slot * index + slot / 2}" cy="${y(value)}" r="4" fill="${color}"/>`
          );
        });
      });
    }
