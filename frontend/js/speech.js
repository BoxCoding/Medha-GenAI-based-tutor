/* Audio narration with read-along highlighting.
 *
...
const FEMALE_HINTS = [
  "veena", "lekha", "heera", "isha", "kalpana", "priya", "female", "neerja", "swara",
];
  ...
  while ((node = walker.nextNode())) {
    ...
    if (remainingStart < textLength) {
      ...
      try {
        ...
        /* boundary landed on a node seam — skip this word */
      }
      return;
    }
...
function collectBlocks(container) {
  ...
  return [...container.querySelectorAll(selector)].filter((element) => {
    ...
    return element.innerText.trim().length > 0 && !element.querySelector(selector);
  });
}
...
export function initSpeech() {
  ...
  voiceSelect.addEventListener("change", () => {
    try {
      localStorage.setItem("medha-voice", voiceSelect.value);
    } catch {
      /* ignore */
    }
  });
